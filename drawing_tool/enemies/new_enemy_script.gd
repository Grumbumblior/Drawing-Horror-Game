...
func CheckForPlayer():
	...
	var result = space_state.intersect_ray(PhysicsRayQueryParameters3D.create($Head.global_position, player.get_node("Head2/Camera3d").global_position, 1, [self.get_rid()]))
...
#				currentState = States.hunting
#				navigationAgent.set_target_position(item.global_position)
	pass
